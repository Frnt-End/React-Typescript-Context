...
const FormList: React.FC<IProps> = ({ setUsers, users, maxlength }) => {
  ...
  const formOk = () => {
    if (users.length < 5)
      if (!input.img || !input.name || !input.position || !input.note) return;
  ...
  return (
    <>
      ...
      <div
        className={
          showForm ? "list-form form-show" : "list-form inactive form-hide"
        }
      >
        <h1>Create Your Winning Team!</h1>
        <h2>
          Within giving you the chance to build your own team (without any
          limitation whatsoever!)..what your Elite Team would look like?
        </h2>
        ...
        <div className="radio-avatar">
          <div>
            <input
              type="radio"
              value={avatarUrl.young}
              name="img"
              onChange={handleChange}
              defaultChecked={true}
            />
            <label htmlFor="Male">
              <img className="thumb-avatar" alt="" src={avatarUrl.young} />
            </label>
          </div>
          ...
          <div>
            <input
              type="radio"
              value={avatarUrl.female}
              name="img"
              onChange={handleChange}
            />
            <label htmlFor="Female">
              {" "}
              <img className="thumb-avatar" alt="" src={avatarUrl.female} />
            </label>
          </div>
          ...
          <div>
            <input
              type="radio"
              value={avatarUrl.adult}
              name="img"
              onChange={handleChange}
            />
            <label htmlFor="Other">
              <img className="thumb-avatar" alt="" src={avatarUrl.adult} />
            </label>
          </div>
          ...
          <div>
            <input
              type="radio"
              value={avatarUrl.other}
              name="img"
              onChange={handleChange}
            />
            <label htmlFor="Other">
              <img className="thumb-avatar" alt="" src={avatarUrl.other} />
            </label>
          </div>
          ...
          <div>
            <input
              type="radio"
              value={avatarUrl.unique}
              name="img"
              onChange={handleChange}
            />
            <label htmlFor="Other">
              <img className="thumb-avatar" alt="" src={avatarUrl.unique} />
            </label>
          </div>
        </div>
